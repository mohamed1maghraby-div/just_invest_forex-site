...
?>
    ...
    top: 0px;"><a class="logo" href="https://www.investorsstate.com"><img src="image/logo.PNG" width="200" height="80"/></a></li>
      ...
      <?php
                    ...
                    $username = $user_coo;
                    $amount = @$_POST['amount'];
                    $u_email_bank_account = @$_POST['u_email_bank_account'];
                    $number_of_deposits = @$_GET['number_of_deposits'];
                    $name_of_bank = @$_POST['name_of_bank'];
                    date_default_timezone_set("America/New_York");
                    $u_date = date("Y-m-d");
                    $a_conferm = 'f';

$checkwithdraw = "select * from deposits where u_username = '$username' and number_of_deposits='$number_of_deposits'";
              $run_checkwithdraw = mysqli_query ($con, $checkwithdraw);
              $fetch_checkwithdraw = mysqli_fetch_object($run_checkwithdraw);
                  $check = $fetch_checkwithdraw->weekly_earings;
if(isset($_POST['withdrawal'])){
    if(empty($username) || empty($amount)){
        echo '<script>alert("please complete the fields");</script>';
    }else{
        if($check >= $amount){
        $insert_w = "insert into pendingwithdrawals(u_username,u_email_bank_account,name_of_bank,weekly_earings,u_date,number_of_deposits,a_conferm) values('$username','$u_email_bank_account','$name_of_bank','$amount','$u_date','$number_of_deposits','$a_conferm')";
        $run_w = mysqli_query($con, $insert_w);
        if(isset($run_w)){
            echo '<script>alert("your withdrew request under processing now");</script>';
        }
            $end_of_calc = $check - $amount ;
            $make_weekly_zero = "UPDATE deposits SET weekly_earings = '$end_of_calc' WHERE u_username = '$username' and number_of_deposits='$number_of_deposits'";
            $run_make_weekly_zero = mysqli_query($con, $make_weekly_zero);
            header( "refresh:2;url=dashbord.php" );
        }else{
            echo '<script>alert("you dont have enghout money is this deal");</script>';
        }
    }
}
?>
            ...
            <div class="dashboard col-lg-12">
                <div class="dashboard_border col-lg-push-1 col-lg-10">
                    <div class="dashbord_inner col-lg-push-1 col-lg-10">
                        ...
                        <div class="ac_wi_ref">
                            THE AVAILABLE AMOUNT FOR WITHDRAWAL: <?php echo $check; ?> usd
                            ...
                            <div class="col-lg-4" style="margin-top: 20px;">
                                Reuests for withdrawal will be processed <br/> after confirmation 
                            </div>
                            <div class="col-lg-4" style="margin-top: 20px;">
